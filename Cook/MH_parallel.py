...
extern "C"
{
    ...
    __device__ void to_shared(ushort *A_glob, ushort *A_shr, ushort size, ushort threadIdLoc) {
    ...
    __device__ void dummy(ushort *vtd_pop, ushort *vtd_clr, ushort *vtd_cmp) {
        ushort w;
        printf("");
        for(w = 0; w < NUM_VTDS; w++) {
            if(vtd_cmp[w] == 0) {
...
*/
    ...
    __device__ void get_cmp(ushort w, ushort *nbrs, ushort *nbr_rng, ushort *vtd_pop, ushort *vtd_clr, ushort *vtd_cmp, ushort *num_cmps) {
        ...
        //printf("\\nSTART OF CMP ASSIGNMENT PROCESS FOR ");
        //print_vtd(w, vtd_pop, vtd_clr, vtd_cmp);
        //printf("\\n");
        orig_cmp = vtd_cmp[w];        
        ...
        if(count > 1) {
            dummy(vtd_pop, vtd_clr, vtd_cmp);
            //printf("   Split component\\n");
            //print_cmps(vtd_pop, vtd_clr, vtd_cmp, (*num_cmps));
            //printf("I just reassigned ");
            //print_vtd(w, vtd_pop, vtd_clr, vtd_cmp);
            //printf(" FROM orig_cmp=%u.\\n", orig_cmp);            
            ...
            for(v = 0; v < NUM_VTDS; v++) {
                if(vtd_cmp[v] == orig_cmp) {                    
                    recmp_vtd(v, cutoff + k, vtd_pop, vtd_clr, vtd_cmp);
                    //printf("  ");
                    k += 1;
                }
    ...
    __global__ void gerrymander_kernel(ushort *vtd_edge0_glob, ushort *vtd_edge1_glob, ushort *nbrs_glob, ushort *nbr_rng_glob, ushort *vtd_pop_glob, uint *rand_seeds, float anneal_rate, ushort *vtd_clr_glob, ushort *num_cmps_glob, float *TV_glob, float *good_glob) {
        ...
        __shared__ ushort vtd_edge0[NUM_EDGES];
        to_shared(vtd_edge0_glob, vtd_edge0, NUM_EDGES, threadIdLoc);

        __shared__ ushort vtd_edge1[NUM_EDGES];
        to_shared(vtd_edge1_glob, vtd_edge1, NUM_EDGES, threadIdLoc);
        ...
        float TV_best = 2;
        ushort num_cmps_best = NUM_CLRS;
        
        
        //print_cmps(vtd_pop, vtd_clr, vtd_cmp, num_cmps);
        //printf("DIV2DIV2");
        for(step = 0; step < MAX_STEPS; step++) {
            //printf("START MARKOV STEP %u\\n", step);
        ...
        num_cmps_glob[threadIdGlob] = num_cmps_best;
        TV_glob[threadIdGlob] = TV_best;
...
num_cmps_gpu = togpu(num_cmps,'uint16')
TV_gpu = togpu(TV,'float32')
...
gerrymander(vtd_edge0_gpu, vtd_edge1_gpu, nbrs_gpu, nbr_rng_gpu, vtd_pop_gpu, rand_seeds_gpu, np.float32(anneal_rate), vtd_clr_arr_gpu, num_cmps_gpu, TV_gpu, good_gpu, block=blockDims, grid=gridDims, shared=0)


vtd_clr_arr = vtd_clr_arr_gpu.get()
vtd_clr_arr = vtd_clr_arr.reshape([num_sims,num_vtds])
num_cmps = num_cmps_gpu.get()
TV = TV_gpu.get()
good = good_gpu.get()
