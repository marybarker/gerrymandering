/* Adobe Analytics code updated 2016-11-14 */

var sc_code_ver="s_code 2.3";
/* Specify the Report Suite ID(s) to track here */
var s_account="",s_channel="",s_filters="census.gov",s_server="";
var curURL=''+window.location.href;
curURL=curURL.toLowerCase();
if(curURL) {
 if(curURL.indexOf('qfembed=')>-1 || curURL.indexOf('ezsts=')>-1 || curURL.indexOf('cngdst=')>-1 || curURL.indexOf('/popclock/embed.php')>-1 || curURL.indexOf('worldpopembed=')>-1 || curURL.indexOf('gloss=')>-1 || curURL.indexOf('/econdashboard/?econind=')>-1) {s_account='cebuwebapps' ;}
 else if(curURL.indexOf('quickfacts.')>-1 || curURL.indexOf('/quickfacts')>-1) {s_account='cebuquickfactsprod';s_server="QuickFacts";}
 else if(curURL.indexOf('factfinder.')>-1 || curURL.indexOf('factfinder2.')>-1) {s_account='cebufactfinderprod';s_server="American FactFinder";}
 else if(curURL.indexOf('ces.census.gov')>-1 || curURL.indexOf('did.census.gov')>-1) {s_account='cebulehd';s_server="LEHD";}
 else if(curURL.indexOf('/did/www/sahie/data/interactive/')>-1) {s_account='cebusahie' ;}
 else if(curURL.indexOf('/did/www/saipe/data/interactive/')>-1) {s_account='cebusaipe' ;}
 else if(curURL.indexOf('census.gov/glossary')>-1) {s_account='cebuwebglossaryprod' ;}
 else if(curURL.indexOf('census.gov')>-1) {s_account='cebucensusglobalprod';}
}
if(s_account!="" && s_account!='cebuwebapps' && s_account!='cebucensusglobalprod') {s_account=s_account+",cebucensusglobalprod";}
if(!s_account) {s_account="cebucensusglobaldev";s_server="Dev";}
...
s.visitorNamespace="censusbureau"
s.trackingServer="censusbureau.d1.sc.omtrdc.net"
 
/* TRACK DOWNLOAD LINKS CONFIG*/
s.trackDownloadLinks=true
s.linkDownloadFileTypes="exe,zip,wav,mp3,mov,mpg,avi,wmv,pdf,doc,docx,xls,xlsx,ppt,pptx,jar,swf,bin,csv,txt,sas7bdat,jpg,jpeg,png,gif,gz"
 
/* TRACK EXIT LINKS CONFIG*/
s.trackExternalLinks=true
s.linkInternalFilters="javascript:," + s_filters;
...
function s_doPlugins(s) {
 ...
 /* External Campaigns */
  if(!s.campaign){
  s.campaign=s.getQueryParam('utm_source,utm_term,utm_content,utm_campaign','|');
  s.campaign=s.getValOnce(s.campaign,'s_utm',0);
 }
 
 /* For PopClock Embeds */
 if(window.location.href.indexOf('popclk')>-1) {
  var s_popclockID = s.getQueryParam('popclk');
  if(s_popclockID){s_server="PopClock";s_channel=s_popclockID;}
 }

 /* For Country-specific World PopClock Embeds */
 if(window.location.href.indexOf('worldpopembed')>-1) {
  var s_worldpopclockID = s.getQueryParam('worldpopembed');
  if(s_worldpopclockID){s_server="WorldPopClock";s_channel=s_worldpopclockID;}
 }

 /* For EasyStats Embeds */
 if(window.location.href.indexOf('ezsts')>-1) {
  var s_easystatID = s.getQueryParam('ezsts');
  if(s_easystatID){s_server="EasyStats";s_channel=s_easystatID;}
 }

 /* For MyCongressionalDistrict Embeds */
 if(window.location.href.indexOf('cngdst')>-1) {
  var s_MyCDID  = s.getQueryParam('cngdst');
  if(s_MyCDID){s_server="My Congressional District";s_channel=s_MyCDID;}
 }

 /* For CIDR Web App Program Views */
 if(window.location.href.indexOf('census.gov/econ/currentdata/dbsearch')>-1) {
  s.eVar23 = s.getQueryParam('program');
 }

  /* For Glossary Embeds */
 if(window.location.href.indexOf('?gloss=')>-1) {
  var s_glossaryID = s.getQueryParam('gloss');
  if(s_glossaryID){s_server="Glossary";s_channel=s_glossaryID;}
 }

  /* For QuickFacts Embeds */
 if(window.location.href.indexOf('?qfembed=')>-1) {
  var s_qfID = s.getQueryParam('qfembed');
  if(s_qfID){s_server="QuickFactsApp";s_channel= s_qfID;}
 }

  /* For Economic Indicators Embeds */
 if(window.location.href.indexOf('?econind=')>-1) {
  var s_econindID = s.getQueryParam('econind');
  if(s_econindID){s_server="Economic Indicator";s_channel=s_econindID;}
 }
...
/************* VISITOR CONVERSION *************/
 ...
 /* External Campaigns */
 s.campaign=s.getValOnce(s.getQueryParam('cid'),'s_camp',0);
 
 /* Internal Campaigns */
 s.eVar1=s.getValOnce(s.getQueryParam('intcmp'),'s_Icamp',0);
 
 /* Internal Search */
 s.eVar19=s.getQueryParam('query');
 
 /* Enhanced download tracking */
 var s_url=s.downloadLinkHandler();
  if(s_url){
   s.eVar3=s_url.substring(s_url.lastIndexOf("/")+1,s_url.length);
   s.events="event2";
   s.linkTrackVars="eVar3,events";
   s.linkTrackEvents=s.events;
 }
 
 /* Email as Referrer Type */
   if(window.location.href.indexOf('eml=')>-1) {
 if (!document.referrer || document.referrer.match(/[^\.]*\.?govdelivery\.com\/?$/i) > -1) {
  var s_eml=s.getValOnce(s.getQueryParam('eml'),'s_email',0);
  if(s_eml){s.referrer="mail://email.campaign/"+s_eml;}
 }
   }
 
/************* VISITOR SEGMENTATION *************/
 s.eVar11=s.getClientTime("t","15");
 s.eVar12=s.getClientTime("d");
 s.eVar13=s.getNewRepeat(60,'s_gnr');
 s.eVar14=s.getDaysSinceLastVisit();
 s.eVar15=s.getVisitNum();

/************* GENERAL VARIABLES *************/
 s.prop51=sc_code_ver;
 s.eVar51="D=g";
 s.plugins=false;
}
...
/*
* Function - sample at a given rate
*/
s.doSampling = new Function("r",""
+"var now=new Date();var time=now.getTime();time+=(3600*1000*24*365*1"
+"0);now.setTime(time);if(!s.c_r('s_smpl')){s.c_w('s_smpl',Math.floor"
+"((Math.random()*100)+1),now);}var smpl=s.c_r('s_smpl');if(smpl%r==0"
